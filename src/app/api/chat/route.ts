...
6. If the user asks to "filter" the dashboard or display specific issues in the UI, you MUST use the apply_dashboard_filters tool. If they ask to filter by a category that exists in the Current Config, you MUST use the 'category' parameter. Only use the 'search' parameter for specific text, phrases, or keywords (e.g., "nvidia crashes") that are not standard categories. Do not just query the database; you must call apply_dashboard_filters so the user's screen updates.`
      },
