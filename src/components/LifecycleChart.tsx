...
export default function LifecycleChart({ clusterId, projectId }: { clusterId: string; projectId: string }) {
  ...
  const statusChartData = data.status_events.map(event => {
    const isClosed = ["fixed", "resolved", "closed_false_positive"].includes(event.status);
    return {
      date: new Date(event.changed_at).toISOString().split("T")[0],
      state: isClosed ? 0 : 1,
  ...
  return (
    <div className={styles.container} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div>
        <h3 className={styles.title}>Close / Reopen Lifecycle</h3>
        <div className={styles.chartWrapper}>
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={statusChartData} margin={{ top: 20, right: 30, left: -20, bottom: 0 }}>
              ...
              <YAxis 
                ...
                domain={[0, 1]}
                ticks={[0, 1]}
                tickFormatter={(val) => val === 1 ? "Open" : "Closed"}
              />
              <Tooltip 
                contentStyle={{ backgroundColor: "#1e293b", borderColor: "#334155", color: "#f8fafc" }}
                formatter={(value: any, name: any, props: any) => [props.payload.statusLabel, "Status"]}
      ...
      <div>
        <h3 className={styles.title}>Similar Issues Reported (Volume)</h3>
        <div className={styles.chartWrapper}>
